...
@push('styles')
    <style>
        ...
        .searchbar {
            ...
            background: #fff;
            position: relative;
            /* agar dropdown bisa absolute ke container */
        }
        ...
        [data-theme="dark"] .chip {
            background: rgba(211, 90, 90, .15);
            color: #ffdcdc;
            border-color: rgba(211, 90, 90, .35);
        }
        ...
        .checkmark-circle::after {
            ...
            margin-top: -4px;
            animation: drawCheck .35s ease-out .15s forwards;
            opacity: 0;
        }
...
@section('content')
        ...
        <div class="cardx">
            ...
            <div class="bd">
                ...
                <div class="searchbar mb-3">
                    ...
                    <div class="suggest-wrap">
                        <input id="scanInput" type="text"
                            placeholder="Ketik / scan barcode (Ctrl+/) — Enter untuk tambah">
                        <!-- dropdown suggestion -->
                ...
                <div class="d-flex flex-wrap gap-2 mt-3">
                    <button class="btn btn-ghost" id="btnHold">
                        <i data-feather="pause"></i> Tunda
                    </button>
                    <button class="btn btn-ghost" id="btnNew">
                        <i data-feather="file-plus"></i> Baru
                    </button>
                    <button class="btn btn-ghost" id="btnRemoveAll">
            ...
            <div class="bd">
                ...
                <div class="total-box mb-3">
                    <div class="rowx">
                        ...
                        <div class="num" id="vSubtotal">0</div>
                    </div>
                    <div class="rowx">
                        <div class="lbl">Diskon Nota</div>
                        <div class="d-flex gap-2 align-items-center">
                            <input id="discBill" type="number" class="form-control form-control-sm" min="0"
                                value="0" style="width:100px; text-align:right">
                            <span class="muted">Rp</span>
                        </div>
                    </div>
                    <div class="rowx">
                        <div class="lbl">PPN</div>
                        <div class="d-flex gap-2 align-items-center">
                            <input id="taxPct" type="number" class="form-control form-control-sm" min="0"
                                step="0.01" value="0" style="width:100px; text-align:right">
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input id="cashIn" type="number" class="form-control" min="0" step="1"
                            value="0">
                    </div>
                    <div class="rowx mt-2">
                        <div class="lbl">Kembali</div>
                        <div class="num fw-bold" id="vChange">0</div>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        <button class="btn btn-outline-secondary pill" data-method="CASH" type="button">Tunai
                        </button>
                        <button class="btn btn-outline-secondary pill" data-method="TRANSFER" type="button">Transfer
                        </button>
                        <button class="btn btn-outline-secondary pill" data-method="QRIS" type="button">QRIS
                        </button>
                        <button class="btn btn-outline-secondary pill" data-method="DEBIT" type="button">Debit
                        </button>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2">
                    <button class="btn btn-brand pay-btn" id="btnPay" type="button">
                        <i data-feather="check-circle"></i> Bayar & Simpan (Enter)
                    </button>
                    <button class="btn btn-outline-secondary pay-btn" id="btnPayNonCash" type="button">
                        <i data-feather="send"></i> Non Tunai
                    </button>
                </div>
            </div>
        </div>
    ...
    {{-- Overlay sukses transaksi --}}
    <div id="successOverlay" class="success-overlay">
        <div class="success-box">
            <div class="checkmark-circle"></div>
            <div class="success-title">Transaksi selesai</div>
            <div class="success-sub">Data penjualan sudah tersimpan.</div>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const money = v =>
                new Intl.NumberFormat('id-ID').format(Math.max(0, Math.round(+v || 0)));
            ...
            function showSuggest() {
                if (sugRows.length) suggestBox.style.display = 'block';
            }
            ...
            scanInput.addEventListener('keydown', e => {
                ...
                if (e.key === 'ArrowDown') {
                    e.preventDefault();
                    if (sugRows.length) {
                        sugIndex = (sugIndex + 1) % sugRows.length;
                        renderSuggest(sugRows);
                    }
                }
                ...
                if (e.key === 'ArrowUp') {
                    e.preventDefault();
                    if (sugRows.length) {
                        sugIndex = (sugIndex - 1 + sugRows.length) % sugRows.length;
                        renderSuggest(sugRows);
                    }
                }
            ...
            document.addEventListener('click', ev => {
                if (!suggestBox.contains(ev.target) && ev.target !== scanInput) {
                    hideSuggest();
                }
            });
                    ...
                    <td class="num">
                        <div class="qty-box ms-auto">
                            <button class="btn btn-sm btn-outline-secondary"
                                    data-act="dec" data-i="${i}">−</button>
                            <input type="number"
                                   class="form-control form-control-sm"
                                   data-act="qty" data-i="${i}"
                                   min="1" step="1" value="${it.qty}">
                            <button class="btn btn-sm btn-outline-secondary"
                                    data-act="inc" data-i="${i}">+</button>
                        </div>
                    </td>
                    <td class="num">
                        <div class="input-group input-group-sm">
                            <input type="number"
                                   class="form-control"
                                   data-act="disc" data-i="${i}"
                                   min="0" step="1" value="${it.disc || 0}">
                            <span class="input-group-text">Rp</span>
                        </div>
                    </td>
                    <td class="num fw-bold">
                        ${money((it.price * it.qty) - (it.disc || 0))}
                    </td>
                    <td class="num">
                        <button class="btn btn-sm btn-outline-danger"
                                data-act="del" data-i="${i}">
            ...
            function updateTotals() {
                const sub = cart.reduce(
                    (s, it) => s + (it.price * it.qty) - (it.disc || 0),
                    0
                );
            ...
            $('#cartBody').addEventListener('input', e => {
                ...
                if (el.dataset.act === 'qty') {
                    ...
                    cart[i].qty = Math.max(1, parseInt(el.value || 1, 10));
                    saveCart();
                    updateTotals();
                }

                if (el.dataset.act === 'disc') {
                    const i = +el.dataset.i;
                    cart[i].disc = Math.max(0, +el.value || 0);
                    saveCart();
                    updateTotals();
                }
            });
            ...
            async function addByText(txt) {
                ...
                try {
                    const res = await fetch(
                        "{{ route('products.lookup') }}?q=" + encodeURIComponent(term), {
                            headers: {
                                'Accept': 'application/json'
                            }
                        }
            ...
            $('#btnPayNonCash').addEventListener('click', () => {
