...
@push('styles')
<style>
  .profile-card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:16px}
  .avatar-lg{width:96px;height:96px;border-radius:50%;object-fit:cover;border:2px solid #fff;box-shadow:0 6px 18px rgba(2,6,23,.15)}
  .hint{color:#64748b}
  .grid{display:grid;gap:1rem}
  @media (min-width: 992px){ .grid-2{grid-template-columns:320px 1fr} }
</style>
@endpush
  ...
  <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data">
    ...
    <div class="grid grid-2">
      ...
      <div class="profile-card">
        ...
        <div class="mt-3">
          ...
          @if($u->profile_photo_path)
            <div class="form-check mt-2">
              <input class="form-check-input" type="checkbox" value="1" id="removePhoto" name="remove_photo">
        ...
        <hr class="my-3">
        <div class="hint small">Terakhir diperbarui: {{ optional($u->updated_at)->format('d M Y H:i') ?? '-' }}</div>
      </div>
