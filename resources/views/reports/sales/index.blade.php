...
<div class="tabs">
  ...
  <form method="GET" class="grid grid-6" style="margin-bottom:12px">
    ...
    <input type="text"   name="payment_method" value="{{ $filters['payment_method'] ?? '' }}" placeholder="Metode Bayar (Tunai/Debit/QR...)">
    <input type="text"   name="q"              value="{{ $filters['q'] ?? '' }}"              placeholder="Cari invoice/customer/kasir...">
    ...
    <div style="grid-column:1/-1;display:flex;gap:8px;align-items:center">
      ...
      <a class="pill" href="{{ route('reports.sales.index', array_merge(request()->except(['date_from','date_to']), ['date_from'=>$today ,'date_to'=>$today ])) }}">Hari ini</a>
      <a class="pill" href="{{ route('reports.sales.index', array_merge(request()->except(['date_from','date_to']), ['date_from'=>$wStart,'date_to'=>$wEnd])) }}">Minggu ini</a>
      <a class="pill" href="{{ route('reports.sales.index', array_merge(request()->except(['date_from','date_to']), ['date_from'=>$mStart,'date_to'=>$mEnd])) }}">Bulan ini</a>
    </div>
  </form>
